...
# New inline policy for iam:CreateRole and iam:CreatePolicy
resource "aws_iam_group_policy" "create_role_and_policy" {
  name  = "create_role_and_policy"
  group = aws_iam_group.user_group.name
...
output "user_credentials" {
  value = [for i in range(var.user_count) : {
    ...
    password = nonsensitive(aws_iam_user_login_profile.user_login_profile[i].password)
  }]
}
